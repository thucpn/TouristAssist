...
export function convertMessageContent(
  ...
  const lat = latInput ?? -16.2329162;
  const long = longInput ?? 136.9949153;
  if (!annotations) return `${content}/nCurrent user location: ${lat}, ${long}`;
  return [
    {
      type: "text",
      text: `${content}/nCurrent user location: ${lat}, ${long}`,
    },
